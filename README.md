...
基于 FastAPI + SSE 实现本地 RAG 流式对话服务；设计三阶段检索管线（查询改写 → 混合召回 → RRF 重排），pgvector 向量检索结合 pg_trgm 三元组文本检索提升召回率；分层记忆（短期滑动窗口 + 长期向量化去重 + 会话自动摘要）支持跨会话上下文感知；支持文件/URL/文本多入口入库，Ollama 调用全链路耗时追踪，进程级 LRU 缓存避免重复 Embedding；Next.js BFF 代理 SSE，前端支持 Markdown 渲染（GFM + 语法高亮）与对话导出。
