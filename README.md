...
**测试步骤：**
1. 打开 `http://localhost:3000/ingest`
2. 拖入或点击选择一个 `.docx` / `.xlsx` 文件
3. 点击「上传并入库」，成功后显示创建的片段数
4. 回到主页对话，提问该文件的内容
...
# 或
INFO:root:[Extract] xlsx → 1850 chars, 2 sheets
```
...
基于 FastAPI + SSE 实现本地 RAG 流式对话服务；设计三阶段检索管线（查询改写 → 混合召回 → RRF 重排），pgvector 余弦向量检索结合 pg_trgm 三元组文本检索，HNSW 索引加速大规模向量查询；分层记忆（短期滑动窗口 + 长期向量化去重 + 会话自动摘要）支持跨会话上下文感知，会话按最近活跃时间排序；支持 txt/md/pdf/docx/xlsx/URL/文本七种入库方式（50 MB 上限保护），全链路耗时追踪与进程级 LRU 缓存；Next.js BFF 代理 SSE，前端 user_id 持久化、历史会话 summary 同步、Markdown 渲染、引用内联高亮点击、含来源的对话 Markdown 导出与系统统计面板。
