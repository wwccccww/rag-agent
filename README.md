...
**引用后处理：** 流式结束后对助手全文做**短语对齐**：若某 `[Sk]` 对应片段中的中文词组 / 英文标识符在正文中命中数不足阈值，则移除该标记（并整理「引用：」行多余顿号）。阈值由 `RAG_CITATION_*` 控制；关闭设 `RAG_CITATION_VERIFY=false`。系统提示中同步要求模型勿乱标引用。
...
1. 在 `backend` 目录执行：`python -m unittest tests.test_citation_guard -v`。

**预期输出：** 显示 `Ran 3 tests ... OK`。
