...
- **Frontend**：`frontend/`（Next.js，BFF 统一 `fastapiFetch`，可选 `FASTAPI_API_KEY` 与后端 `API_KEY` 对齐）
...
（可选）若后端 `.env` 中配置了 **`API_KEY`**，在 `.env.local` 增加 **`FASTAPI_API_KEY=<同一密钥>`**，否则 BFF 调用上游会返回 401。

打开 `http://localhost:3000`。

## Docker Compose（Postgres + 后端 + 前端）

在仓库根目录使用 Compose 启动 **pgvector**、**FastAPI**、**Next.js**（BFF 通过 `FASTAPI_BASE_URL=http://backend:8000` 访问后端）。**Ollama 默认指向宿主机** `host.docker.internal:11434`（`backend` 服务已配置 `extra_hosts: host-gateway`，Linux 与 Docker Desktop 均可解析宿主机）。

如果 `host.docker.internal` 仍不可达，请将环境变量 **`OLLAMA_BASE_URL`** 设为实际可访问的 Ollama 地址后重新 `docker compose up`。

```powershell
cd d:\1study\study\python\rag-agent
# 可选：复制 .env 并设置 API_KEY / FASTAPI_API_KEY（两者一致）
docker compose up --build
```

- 数据库：`postgresql+psycopg://rag:ragpass@localhost:5432/rag`（从宿主机连接时端口映射 `5432`）
- API：`http://127.0.0.1:8000`
- 前端：`http://localhost:3000`

**测试步骤：**
1. 执行 `docker compose up --build`，等待 `backend` 与 `frontend` 就绪。
2. 浏览器打开 `http://localhost:3000`，发起一轮 RAG 对话。
3. 访问 `http://127.0.0.1:8000/v1/audit/qa?user_id=demo`（若配置了 `API_KEY`，须带头 `X-API-Key`）或通过 BFF：`http://localhost:3000/api/audit/qa?user_id=demo`。

**预期输出：**
- `GET /v1/audit/qa` 返回 JSON 数组，最近一轮含 `user_message`、`mode`、`cited_chunk_ids` 等字段。
...
| `GET /v1/audit/qa`         | **问答轮次审计**：查询参数 `user_id`（默认 `demo`）、可选 `session_id` / `mode` / `kb_collection` / `request_id`、`limit`（1–500）；返回每轮用户问题、分区、模式、引用 `chunk_id` 列表等 |
...
**Reflection 超时预算（建议开启，避免长尾卡死）：**

- `AGENT_REFLECTION_BUDGET_MS=1500`：Reflection 评估的延迟预算（毫秒）。超过预算会跳过反思并默认视为 `SUFFICIENT`（继续生成），避免出现 300s 的极端长尾。
...
| `API_KEY`                     | *(空)*                                  | **非空**时除 `/`、`/docs*`、`/openapi.json`、`/redoc`、`/v1/health` 外，请求须带 `X-API-Key` 或 `Authorization: Bearer <同值>` |
| `QA_AUDIT_ENABLED`            | `true`                                 | `false` 时不写入 `qa_audit_logs`（问答轮次审计） |
...
**查询方式：**
- **工具审计 UI**：`http://localhost:3000/audit`
- **问答审计 UI**：`http://localhost:3000/audit/qa`（侧栏「💬 问答审计」同步带上当前 `user_id` 查询参数）
- 工具调用 API：`GET /v1/audit/tools?user_id=demo&limit=100`（可选：`tool` / `status` / `request_id` / `session_id`）
- **问答轮次** API：`GET /v1/audit/qa?user_id=demo&limit=100`（可选：`mode` / `kb_collection` / `session_id` / `request_id`）；Next BFF：`GET /api/audit/qa?...`。开启后端 `API_KEY` 时须携带 `X-API-Key`（与前端 `FASTAPI_API_KEY` 一致）；未配置 `API_KEY` 时不校验。

**测试步骤（问答审计页）：**
1. 在首页用任意模式完成至少一轮对话。
2. 打开 `http://localhost:3000/audit/qa`（或侧栏「问答审计」），点击刷新。
3. 表格中应出现对应 `user_id` 的记录，展开「详情」可见完整问题、`assistant_preview` 与 `cited_chunk_ids`。

**预期输出：** 列表非空时可见时间、mode、分区、问题摘要、引用数；详情中含 chunk id JSON 数组。
