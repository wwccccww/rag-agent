...
| `DEEPEVAL_JUDGE_MODEL` | _(空)_ | 运行 `eval/deepeval_rag.py` 时指定评判用 Ollama 模型；为空则使用 `OLLAMA_CHAT_MODEL` |
...
**预期输出：** 终端逐条打印 DeepEval 结果。非拒答用例含 Faithfulness、Answer Relevancy、Contextual Relevancy；拒答用例仅含 Contextual Relevancy，并带 `[NOTE] ... 仅评测 Contextual Relevancy` 说明。
