...
2. 点击顶栏「↓ 导出 MD」按钮
3. 检查下载的 `.md` 文件，文件名为会话标题（最多 20 字符）
...
基于 FastAPI + SSE 实现本地 RAG 流式对话服务；设计三阶段检索管线（查询改写 → 混合召回 → RRF 重排），pgvector 向量检索结合 pg_trgm 三元组文本检索提升召回率；分层记忆（短期滑动窗口 + 长期向量化去重 + 会话自动摘要）支持跨会话上下文感知；支持 txt/md/pdf/docx/xlsx/URL/文本七种入库方式，全链路耗时追踪与进程级 LRU 缓存；Next.js BFF 代理 SSE，前端支持 Markdown 渲染、引用内联高亮点击、对话导出与系统统计面板。
