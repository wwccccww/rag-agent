...
**入库分块：** `.md` 或含 `##` 标题的正文默认**按标题分节**；节内识别 **Markdown `` ``` `` 围栏代码块**：围栏内不按句号/短窗切碎，整块尽量保留；仅当单块围栏仍超过 `CHUNK_MAX_CHARS` 时，在**换行边界**切为多段（子块之间不叠加以避免行内半截标签）。普通正文仍按空行段落合并，长段优先在句号/换行处断开。`meta` 中带 `section_heading` 便于溯源。可调 `CHUNK_MAX_CHARS`、`CHUNK_OVERLAP`、`CHUNK_MARKDOWN_BY_HEADING`、`CHUNK_MARKDOWN_FENCE_AWARE`。
...
**预期输出：** 步骤 1 显示 `Ran 3 tests ... OK`；步骤 2 中各片段内容在代码行语义上完整可读。
