...
/** 与后端 `resolve_kb_collection` 一致：1–64 位 [a-zA-Z0-9_-] */
const KB_COLLECTION_RE = /^[a-zA-Z0-9_-]{1,64}$/;
...
/**
 * 将用户输入规范为 kb_collection；无法得到合法值时返回 null。
 * 与后端 `^[a-zA-Z0-9_-]{1,64}$` 对齐。
 */
export function slugKbCollection(raw: string): string | null {
  let s = raw.trim().slice(0, 64);
  if (!s) return null;
  s = s.replace(/[^a-zA-Z0-9_-]+/g, "-");
  s = s.replace(/-+/g, "-").replace(/^[-_]+|[-_]+$/g, "");
  if (!s || !KB_COLLECTION_RE.test(s)) return null;
  return s;
}
